...
class SSDRotateHead(nn.Module):
    ...
    def get_guided_anchors(self, box_preds, cls_preds, dir_cls_preds, anchors, anchors_mask, gt_bboxes, thr=.1):
        ...
        for box_preds, cls_preds, dir_preds, a_mask, gt_boxes in zip(
            ...
            print("cls_preds, tot scores", cls_preds.shape, total_scores.shape)
            ...
            if self._use_direction_classifier:
                ...
                #print((box_preds[..., -1] > 0).type(), torch.cuda.BoolTensor(list(map(bool,dir_labels)).type())
                opp_labels = (box_preds[..., -1] > 0) ^ torch.cuda.BoolTensor(list(map(bool,dir_labels)))
